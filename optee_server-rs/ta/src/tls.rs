...
pub fn do_tls_read(tls_session: &mut rustls::ServerSession, encrypted_buf: &[u8], plain_buf: &mut Vec<u8>){
...
pub fn do_tls_write(tls_session: &mut rustls::ServerSession, encrypted_buf: &mut [u8], plain_buf: &[u8]) -> usize {
...
fn load_certs() -> Vec<rustls::Certificate> {
    let bytes = include_bytes!("../test-ca/server-cert.pem").to_vec();
...
fn load_private_key() -> rustls::PrivateKey {
    let bytes = include_bytes!("../test-ca/server.key").to_vec();
